typeof navigator === "object" && (function (Sentry, Shr) {
  ...
  const controls = {
    ...
    updateSeekTooltip(event) {
      ...
      const toggle = show => toggleClass$1(tipElement, visible, show); // Hide on touch
    ...
    setMarkers() {
      ...
      const toggleTip = show => toggleClass$1(tipElement, tipVisible, show); // Inject markers to progress container
  ...
  (() => {
    ...
    const isProduction = window.location.host.includes(production); // Sentry for demo site (https://plyr.io) only
